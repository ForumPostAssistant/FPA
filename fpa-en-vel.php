<?php
    ...
    define ( '_FPA_VEL_TVELSTATUS', 'VEL Status');
    define ( '_FPA_VEL_TVELISVULN', 'Confirm this vulnerability report and take appropriate action.' );
    define ( '_FPA_VEL_TVELMAYBEVULN', 'The VEL listing provided no version information.' );
    define ( '_FPA_VEL_NODATA', 'Could not contact VEL, please try again later');
    define ( '_FPA_VEL_NOMATCHES', 'No extensions found on the Vulnerable Extension List');
    define ( '_FPA_VEL_DISCLAIMER', '<strong>Disclaimer:</strong><br />FPA does NOT guarantee the accuracy of this tool, it offers a guildeline only. To ensure accuracy, please manually check you extensions again the VEL & developer websites.' );
    ...
    */
    ...
    if ( defined( '_LIVE_CHECK_VEL') AND $canDOLIVE == '1' AND $instance['instanceFOUND'] == _FPA_Y AND @$_POST['doVEL'] == 1 ) {
        ...
        if ( file_exists('fpa-veldata.json')) {
            // get test data
            $get_data        = file_get_contents('fpa-veldata.json');
            $velcURLARRAY    = json_decode($get_data, true);
            $velITEMARRAY   = $velcURLARRAY['data']['items'];

            echo '<div class="bg-info text-center text-white">DEV MESSAGE : USING VEL TEST DATA</div>';
        } else {
            ...
            if ($velcURLJSON ===  FALSE) {
              ...
              echo '<div class="bg-info text-center text-white">DEV MESSAGE : USING VEL LIVE DATA</div>';
              $velcURLARRAY   = json_decode($velcURLJSON, true);  // decode json in to an array
              $velITEMARRAY   = $velcURLARRAY['data']['items'];
            }
        }

        // did fpa get a result returned from vel.joomla.org
        // no point in doing anything else if not
        if ( $velITEMARRAY ) {
            $gotVELDATA = 1;
        } else {
            $gotVELDATA = 0;
...
<!--VELTESTING-->
<?php if ( $canDOLIVE == '1' AND @$_POST['doVEL'] == 1 ) { ?>
    <div class="container" id="vel-container">

        <div class="row text-white xsmall" id="<?php echo $arranme.'-vel'; ?>">
            <div class="col-5 col-md-3 py-2 bg-info border-right font-weight-bolder">Name</div>
            <div class="col-3 col-md-2 py-2 bg-info border-right text-center font-weight-bolder">Version</div>
            <div class="col-4 col-md-2 py-2 bg-info border-right text-center">Created/Author</div>
            <div class="col-5 d-none d-md-inline-block d-md-5 py-2 bg-info font-weight-bolder">VEL Status</div>
        </div><!--/row #arrname-vel-->
        ...
        <?php if ( $instance['instanceFOUND'] == _FPA_Y ) { ?>
            ...
            <?php if ( isset($component['SITE'])) { ?>
                ...
                <?php foreach ( $component['SITE'] as $key => $show ) { ?>
                    ...
                    <?php
                        ...
                        foreach ($velITEMARRAY as $checkVEL) {

                            // PRODUCTION if ( (stristr( (string)$checkVEL['title'], (string)$show['name'] ) !== false OR stristr( (string)@$checkVEL['description'], (string)$show['name'] ) !== false) AND $show['type'] != _FPA_JCORE ) {
                            if ( $show['type'] != _FPA_JCORE AND (stristr( (string)$checkVEL['title'], (string)$show['name'] ) !== false OR stristr( (string)@$checkVEL['description'], (string)$show['name'] ) !== false)) {
                                ...
                                $velEXTREPORT[$countEXTVELREPORT]['vulnerable_at']   = @$checkVEL['vulnerable_version'];
                                $velEXTREPORT[$countEXTVELREPORT]['patched_at']      = @$checkVEL['patch_version'];
                                $velEXTREPORT[$countEXTVELREPORT]['description']     = strip_tags( html_entity_decode( $checkVEL['description'] ) );
                                $velEXTREPORT[$countEXTVELREPORT]['fix_status']      = @$checkVEL['status'];
                                $velEXTREPORT[$countEXTVELREPORT]['fix_status_text'] = @$checkVEL['statusText'];

                                if ( empty($velEXTREPORT[$countEXTVELREPORT]['started_at']) AND empty($velEXTREPORT[$countEXTVELREPORT]['patched_at']) ) {

                                    $velEXTREPORT[$countEXTVELREPORT]['still_vuln'] = _FPA_M;
                                    $velEXTREPORT[$countEXTVELREPORT]['suggest'] = _FPA_VEL_TVELISVULN .' '. _FPA_VEL_TVELMAYBEVULN;
                                    $isVULN = _FPA_M;

                                } elseif ( ( empty($velEXTREPORT[$countEXTVELREPORT]['started_at']) AND version_compare($velEXTREPORT[$countEXTVELREPORT]['patched_at'], $show['version'], '<=' ) )
                                            OR ( !empty($velEXTREPORT[$countEXTVELREPORT]['started_at']) AND version_compare($velEXTREPORT[$countEXTVELREPORT]['started_at'], $show['version'], '<' ) )
                                            OR ( version_compare($velEXTREPORT[$countEXTVELREPORT]['patched_at'], $show['version'], '<')) ) {

                                    $velEXTREPORT[$countEXTVELREPORT]['still_vuln'] = _FPA_N;
                                    $isVULN = _FPA_N;

                                } else {

                                    $velEXTREPORT[$countEXTVELREPORT]['still_vuln'] = _FPA_Y;
                                    $velEXTREPORT[$countEXTVELREPORT]['suggest'] = _FPA_VEL_TVELISVULN;
                                    $isVULN = _FPA_Y;
                    ...
                    //var_dump($velEXTREPORT);
                    ?>
                    <!--VELSTUFF-->


                    <?php if ( $isONVEL == 1 AND $isVULN != _FPA_N) { ?>
                        <div class="row border border bg-light mb-3" id="<?php echo $arranme.'-veldata'; ?>">
                            <div class="col-5 col-md-3 py-1 text-dark small border-right"><?php echo $show['name']; ?></div>
                            <div class="col-3 col-md-2 py-1 text-dark small border-right text-center"><?php echo $show['version']; ?></div>
                            <div class="col-4 col-md-2 xsmall border-right text-center"><?php echo $show['creationDate']; ?><br /><?php echo $show['author']; ?></div>
                            <div class="col-12 d-md-none py-2 text-white xsmall bg-info font-weight-bolder">VEL Status</div>

                            <!--veldata-->
                            <div class="col-12 col-md-5 py-1 small">

                            <?php foreach($velEXTREPORT as $key) { ?>

                                <?php if ( $key['still_vuln'] != _FPA_N ) {

                                    if ( $key['risk_level'] == 'small' ) {
                                        $colBG = 'info';
                                        $divBorder = 'info';
                                        $iconRisk = 'exclamation-circle';
                                        $textColor = 'white';
                                    } elseif ( $key['risk_level'] == 'medium' ) {
                                        $colBG = 'warning';
                                        $divBorder = 'warning';
                                        $iconRisk = 'exclamation-triangle';
                                        $textColor = 'white';
                                    } elseif ( $key['risk_level'] == 'high' ) {
                                        $colBG = 'danger';
                                        $divBorder = 'danger';
                                        $iconRisk = 'skull-crossbones';
                                        $textColor = 'white';
                                    } else {
                                        $colBG = 'info';
                                        $divBorder = 'info';
                                        $iconRisk = 'question-circle';
                                        $textColor = 'white';
                                    }


                                    if ( empty($key['started_at']) AND empty($key['patched_at']) ) {
                                        $vulnerable_from = '<span class="badge badge-info">'. _FPA_U .'</span>';

                                    } elseif ( empty($key['started_at']) AND !empty($key['patched_at']) ) {
                                        $vulnerable_from = '<span class="xsmall">All before</span> <span class="badge badge-warning">'. $key['patched_at'] .'</span>';

                                    } elseif ( !empty($key['started_at']) ) {
                                        $vulnerable_from = '<span class="badge badge-warning">'. $key['started_at'] .'</span>';

                                    } else {
                                        $vulnerable_from = '<span class="badge badge-warning">'. $key['patched_at'] .'</span>';
                                    }

                                    if ( empty($key['patched_at']) ) {
                                        $patched_at = '<span class="badge badge-info">'. _FPA_U .'</span>';
                                    } else {
                                        $patched_at = '<span class="badge badge-info">'. $key['patched_at'] .'</span>';
                                    }

                                ?>

                                <div class="row no-gutters mb-2">
                                    <div class="col-3 col-md-3 text-center text-white">
                                        <div class=" px-1 py-2 bg-<?php echo $colBG; ?> w-75">
                                            <i class="fas fa-<?php echo $iconRisk; ?> fa-lg"></i>
                                            <span class="d-block xsmall pt-2">
                                                <?php
                                                    if ( @$key['risk_level'] ) {
            ...
            <?php } // isset extensionType ?>

        <?php } // instanceFOUND ?>

    </div><!--/vel container-->

    <div class="bg-light border mb-2">
        <p class="xsmall text-center py-1 mb-0"><?php echo _FPA_VEL_DISCLAIMER; ?></p>
    </div>
...
<?php if ( $canDOLIVE == '1' AND @$_POST['doVEL'] == 1 ) { ?>
    ...
    <div class="col-12 bg-light border mb-2">
        <p class="xsmall text-center py-1 mb-0"><?php echo _FPA_VEL_DISCLAIMER; ?></p>
    </div>
